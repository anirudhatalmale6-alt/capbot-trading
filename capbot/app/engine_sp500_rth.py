...
def tz_safe(x, tz):
    """
    Safe timezone attach/convert:
      - if tz-naive -> tz_localize(tz)
      - if tz-aware -> tz_convert(tz)
    Works for pandas Timestamp/DatetimeIndex.
    """
    try:
        cur = getattr(x, "tz", None)
        if cur is None:
            return tz_safe(x, tz)
        return x.tz_convert(tz)
    except TypeError:
        # pandas raises TypeError when tz_localize called on tz-aware
        return x.tz_convert(tz)
...
_VIS_GATE_LAST_LOG = {"ts": 0.0}  # vis outside-rth throttle

_GATE_SP500_LAST_LOG = {"ts": 0.0}  # gate log throttle
...
def _rth_is_open(rth: RTH, ts: pd.Timestamp) -> bool:
    # --- guard: accept non-Timestamp values without crashing ---
    try:
        import pandas as pd
        if not isinstance(ts, pd.Timestamp):
            # Sometimes we receive ints (e.g. row index / counter). Use 'now' to avoid tz_convert crash.
            if isinstance(ts, (int, float)) and ts < 10**12:
                ts = tz_safe(pd.Timestamp.utcnow(), "UTC")
            else:
                ts = pd.Timestamp(ts)
    ...
    # fallback manual (accepts int/float/str epoch too)
    import pytz
    import pandas as pd
    ...
    # if ts is not a pandas Timestamp with tz_convert, normalize it
    ...
    # normalize ts (can be epoch int/float)
    import pandas as pd
    if ts is None:
        return False
    if not hasattr(ts, 'tz_convert'):
    ...
    try:
        ...
        while True:
            ...
            if rth_enabled:
                try:
                    if not _rth_is_open(rth, now):
                        try:
                            now_ts = time.time()
                            if (now_ts - _GATE_SP500_LAST_LOG["ts"]) >= 900:
                                log_line(logfile, f"GATE_SP500: outside RTH ({rth_start}-{rth_end} {tz_name}) now_utc={now}")
                                _GATE_SP500_LAST_LOG["ts"] = now_ts

                                now_ts2 = time.time()
                                if (now_ts2 - _VIS_GATE_LAST_LOG["ts"]) >= 30:
            ...
            if in_position:
                ...
                if rth_enabled and (not _rth_is_open(rth, sig_ts)):
            ...
            if disable_thursday_utc and sig_ts.tz_convert('UTC').weekday() == 3:
            ...
            if rth_enabled and (not _rth_is_open(rth, sig_ts)):
                log_line(logfile, f"GATE: outside RTH ({rth_start}-{rth_end} {tz_name}) sig_ts={sig_ts}")
            ...
            try:
                if str(bot_id) in ("sp500_5m","sp500_5m_rth"):
                    ...
                    if df_vis is None or df_vis.empty or len(df_vis) < 3:
                        ...
                        # VIS_OUTSIDE_DEFAULT_FLAGS (evita UnboundLocalError)
                        rL = '❌'
                        rS = '❌'
                        aok = '❌'
                        # VIS_OUTSIDE_DEFAULT_FLAGS_FIXED (evita UnboundLocalError)
                        rsi_long_ok = False
                        rsi_short_ok = False
                        atr_ok = False
                        log_line(
                            logfile,
                            "VIS_NY | sigNY={ny} | THU {thu} | RTH {rth} | VALID {val} | BODY {b}({bv}>={bmin}) | VOL {v}({vv}>={vmin}) | RSI {rv} | ATR {av}".format(
                                ny=ny_str,
...
# --- signal call (compat) ---
            fn = getattr(strat, "signal_on_bar_close", None)
            if callable(fn):
                sig = fn(df, strat_params)
            else:
                fn2 = getattr(strat, "signal_on_row", None)
                if callable(fn2):
                    # last row as dict
                    try:
                        ...
                        row = {}
                    sig = fn2(df, strat_params)
                else:
                    cand = [a for a in dir(strat) if "signal" in a.lower()]
                    raise AttributeError(f"Strategy missing signal method. Need signal_on_bar_close or signal_on_row. Found: {cand}")
