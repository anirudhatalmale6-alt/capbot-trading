...
from capbot.app.engine import log_line
...
class DE40VWAPK020:
    ...
    def enrich(self, df: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
        ...
        out["body_ratio"] = out["body_ratio"].replace([np.inf, -np.inf], np.nan)
    ...
    def signal_on_bar_close(self, df: pd.DataFrame, params: Dict[str, Any]) -> Optional[Signal]:
        ...
        if DEBUG:
            try:
                ...
                vol_ok = vol_rel >= VOL_REL_MIN
                long_rsi_ok = rsi <= RSI_LONG_MAX
                short_rsi_ok = rsi >= RSI_SHORT_MIN
                vwap_gate = abs(close - vwap) >= (VWAP_DISTANCE_K * atr)
                bears_ok = bear_prev3 >= BEAR_PREV3_LONG
                bulls_ok = bull_prev3 >= BULL_PREV3_SHORT

                # OJO: no tenemos logfile aquí; log_line soporta logfile=None -> stdout/journal si está así en tu repo
                log_line(None,
                    "CHECK "
                    f"ts_local={ts_local} "
                    f"body={body_ratio:.2f}>={BODY_MIN}({body_ok}) "
                    f"vol_rel={vol_rel:.2f}>={VOL_REL_MIN}({vol_ok}) "
                    f"rsi={rsi:.1f} long_max={RSI_LONG_MAX}({long_rsi_ok}) short_min={RSI_SHORT_MIN}({short_rsi_ok}) "
                    f"atr={atr:.2f} "
                    f"vwap={vwap:.2f} close={close:.2f} dist={abs(close-vwap):.2f} gate={VWAP_DISTANCE_K*atr:.2f}({vwap_gate}) "
                    f"bear_prev3={bear_prev3:.0f}>={BEAR_PREV3_LONG}({bears_ok}) "
                    f"bull_prev3={bull_prev3:.0f}>={BULL_PREV3_SHORT}({bulls_ok})"
                )
            except Exception as e:
                log_line(None, f"CHECK warning: {repr(e)}")
        ...
        # entry_price_est requerido por Signal (usamos el open de la vela actual [-1] como estimate del 'next open')
        try:
